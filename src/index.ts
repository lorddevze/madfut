...
let packs: Max3Array<{pack: string, amount: number}> = [
    {pack: "query,Madfut Melonfut,,64,100,-1,-1,-1,false,100", amount: 1},
    {pack: "query,Madfut Melonfut,,94,100,-1,-1,-1,false,100", amount: 1},
    // "query,madfut_icon,,64,100,-1,-1,-1,false,100"
    {pack: "query,Madfut Melonfut,,90,100,-1,-1,-1,false,100", amount: 1},
];
...
bot.on("wallet", async (interaction, page) => {
    ...
    interaction.editOriginalMessage({
        embeds: [{
            color: 0x32A852,
            author: {
                name: `${interaction.member!.username}'s Melonfut wallet (page ${page}/${numPages})`,
...
bot.on("deposit", async (interaction, multiple) => {
    ...
    try {
        do {
            ...
            try {
                tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "Melonfut Wallet deposit");
            } catch (err) {
...
async function withdraw(interaction: Eris.CommandInteraction<Eris.TextableChannel>, userId: string, username: string, coins: number, walletVerification: WalletVerificationResult) {
    ...
    while (coinsToGive > 0 || cardsToGive.size > 0 || packsToGive.size > 0) {
        const tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "Melonfut Wallet withdrawal");
...
async function giveawayTrade(username: string) {
    while (giveawayRunning) {
        ...
        try {
            tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "Madfut Melonfut giveaway 🎉");
        } catch {
            console.log(`${username} rejected invite or timed out.`);
            break;
        }
